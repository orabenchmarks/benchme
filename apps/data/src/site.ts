...
export function buildSite(seed: number, scenarioKey = "acme-v1"): SiteFile[] {
  ...
  // the site root ("" on index.html, "../" one directory down, …).
  ...
  /** Adds an HTML page at `path`; `body(r)` receives the page's root-relative prefix for its own links. */
  const html = (path: string, title: string, body: (r: string) => string) => {
    const r = root(path);
    add(path, page(co.name, title, body(r), nav(r)));
  };

  const categories = [...new Set(rows.warehouse.products.map((x) => x.category))].sort();
  html("index.html", "Home", (r) => `<h1>${esc(co.name)}</h1><p>Industrial supply since ${co.founded}, headquartered in ${esc(co.headquarters)}. ${co.employees} employees across ${rows.warehouse.locations.length} depots.</p><ul>${categories.map((c) => `<li><a href="${r}products/${c}.html">${esc(c)}</a></li>`).join("")}</ul>`);
  html("about.html", "About", () => `<h1>About ${esc(co.name)}</h1><table><tr><th>Founded</th><td>${co.founded}</td></tr><tr><th>Headquarters</th><td>${esc(co.headquarters)}</td></tr><tr><th>Employees</th><td>${co.employees}</td></tr><tr><th>Depots</th><td>${rows.warehouse.locations.length}</td></tr><tr><th>Customers</th><td>${rows.warehouse.customers.length}</td></tr></table>`);
  html("locations.html", "Locations", () => `<h1>Depots</h1><table><tr><th>Code</th><th>Name</th><th>City</th></tr>${rows.warehouse.locations.map((l) => `<tr><td>${l.code}</td><td>${esc(l.name)}</td><td>${esc(l.city)}</td></tr>`).join("")}</table>`);

  html("products/index.html", "Products", () => `<h1>Product categories</h1><ul>${categories.map((c) => `<li><a href="${c}.html">${esc(c)}</a> (${rows.warehouse.products.filter((x) => x.category === c).length})</li>`).join("")}</ul>`);
  for (const c of categories) {
    const items = rows.warehouse.products.filter((x) => x.category === c);
    html(`products/${c}.html`, c, () => `<h1>${esc(c)}</h1><table><tr><th>SKU</th><th>Name</th><th>List price</th></tr>${items.map((x) => `<tr><td>${x.sku}</td><td>${esc(x.name)}</td><td>${money(x.unitPriceCents)}</td></tr>`).join("")}</table>`);
  }

  const rev = co.fiscalYearRevenueCents;
  const byQuarter = [0.22, 0.24, 0.26, 0.28].map((share) => Math.round(rev * share));
  html("reports/annual-report.html", "Annual report", () => `<h1>Annual report FY2025</h1><p>Total revenue: <strong>${money(rev)}</strong>.</p><table><tr><th>Quarter</th><th>Revenue</th></tr>${byQuarter.map((q, i) => `<tr><td>Q${i + 1}</td><td>${money(q)}</td></tr>`).join("")}</table><p>Headcount at year end: ${co.employees}.</p>`);
  ...
  html("changelog.html", "Changelog", () => `<h1>Changelog</h1><table><tr><th>Version</th><th>Date</th><th>Change</th></tr>${entries.map((e) => `<tr><td>${e.v}</td><td>${e.date}</td><td>${esc(e.note)}</td></tr>`).join("")}</table>`);

  html("downloads/index.html", "Downloads", () => `<h1>Downloads</h1><ul><li><a href="products.csv">products.csv</a></li><li><a href="locations.csv">locations.csv</a></li><li><a href="customers.csv">customers.csv</a></li></ul>`);
