/**
 * The first ten intent tasks — six authored against the brief, four reads
 * answerable on every surface (reads.mjs) — exactly as tools/intent-corpus.mjs
 * derived them before the template registry (templates/): same picks, same
 * ids, same answer keys, same prose, so nothing a consumer pinned ever moves.
 * New tasks are templates, never edits here.
 *
 *   deriveOriginal(rows, seed) → { problems, specs, tasks, md, claims, solutions, answers }
 *
 * `claims` are the entities these tasks use (templates never reuse them);
 * `solutions` perform each state task through the apps' REST API and
 * `answers` read each json task's answer back from the live apps — both for
 * tools/intent-integrity.mjs, never shipped in the corpus.
 */
import { documentsMatching, lowStock, stockOf } from "../../packages/scenarios/dist/index.js";
import { deriveReads } from "./reads.mjs";

export function deriveOriginal(rows, SEED) {
  const problems = [];
  ...
  if (problems.length) return { problems, specs: [], tasks: [], md: "", claims: [], solutions: {}, answers: {} };
  ...
  // The corpus header (task count, seed) is corpus.mjs's; this is the body
  // it always carried, below that header.
  const md = `Two REST-shape facts, true for every task below, are worth stating once:
...
`;
  ...
  const claims = [
    `customer:${orderCustomer.code}`,
    `sku:${orderProduct.sku}`,
    `sku:${transferSrc.sku}`,
    `ticket:${ticketCandidate.ticketNo}`,
    `stock-read:${stockProduct.sku}`,
    `lowstock:${LOW_THRESHOLD}`,
    `doc-read:${docPick.doc.id}`,
    ...reads.claims,
  ];
  const solutions = {
    "intent-order-01": (api) => api.post("warehouse", "/api/v1/orders", { customer: orderCustomer.code, lines: [{ sku: orderProduct.sku, qty: ORDER_QTY }] }),
    "intent-transfer-01": async (api) => {
      const t = await api.post("warehouse", "/api/v1/transfers", { sku: transferSrc.sku, from: transferSrc.locationCode, to: transferTo, qty: TRANSFER_QTY });
      await api.post("warehouse", `/api/v1/transfers/${t.transferNo}/complete`, {});
    },
    "intent-ticket-01": async (api) => {
      await api.post("helpdesk", `/api/v1/tickets/${ticketCandidate.ticketNo}/assign`, { agentCode: ticketAgent.code });
      await api.post("helpdesk", `/api/v1/tickets/${ticketCandidate.ticketNo}/status`, { status: "resolved" });
    },
  };
  const answers = {
    "intent-stock-01": async (api) => ({ qty: (await api.get("warehouse", `/api/v1/stock?sku=${stockProduct.sku}`)).total }),
    "intent-lowstock-01": async (api) => ({ skus: (await api.get("warehouse", `/api/v1/stock/low?threshold=${LOW_THRESHOLD}`)).map((r) => r.sku).sort().join(",") }),
    "intent-doc-01": async (api) => ({ documentId: await api.documentTitled(docPick.phrase) }),
    ...reads.answers,
  };
  return { problems, specs, tasks, md: md + reads.md, claims, solutions, answers };
}
