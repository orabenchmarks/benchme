...
export type NewWorkspace = Pick<Workspace, "id" | "scenario" | "seed" | "expiresAt">;
...
type Row = { id: string; scenario: string; seed: number; created_at: Date; expires_at: Date; finalized_at: Date | null };

function toWorkspace(r: Row): Workspace {
  return { id: r.id, scenario: r.scenario, seed: r.seed, createdAt: r.created_at, expiresAt: r.expires_at, finalizedAt: r.finalized_at };
}
...
export class PgWorkspaceRepo implements WorkspaceRepo {
  ...
  async create(input: NewWorkspace): Promise<Workspace> {
    const r = await this.pool.query<Row>(
      `INSERT INTO core.workspaces (id, scenario, seed, expires_at) VALUES ($1, $2, $3, $4) RETURNING *`,
      [input.id, input.scenario, input.seed, input.expiresAt],
    );
